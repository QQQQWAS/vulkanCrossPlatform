...
void pbMain(BaseState& appState){
  ...
  std::vector<void*> uniformMaps(FRAMES_IN_FLIGHT);
  for(int i = 0; i < FRAMES_IN_FLIGHT; i++){
    uniformBuffers[i].create(device, physicalDeviceMemProperties, sizeof(glm::mat4), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
    uniformMaps[i] = uniformBuffers[i].map();
  }
  
  for(int i = 0; i < FRAMES_IN_FLIGHT; i++){
    vkUpdateDescriptorSets(device, 1, Ptr(VkWriteDescriptorSet{
  ...
  while(appState.pollEvents()){
    ...
    glm::mat4* transformUniform = (glm::mat4*)uniformMaps[frameInFlightIdx];
    *transformUniform = glm::rotate(glm::mat4(1), (float)time, glm::vec3(0, 0, 1));
